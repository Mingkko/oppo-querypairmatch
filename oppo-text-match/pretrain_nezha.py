...
AdamE = extend_with_exponential_moving_average(Adam)
# AdamELR = extend_with_piecewise_linear_lr(AdamE)

model.compile(loss=masked_crossentropy, optimizer=AdamE(lr=5e-5))
model.summary()

# 转换数据集
train_generator = data_generator(train_data, batch_size)
valid_generator = data_generator(valid_data, batch_size)
test_generator = data_generator(test_data2, batch_size)
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if val_score > self.best_val_score:
            ...
            model.save_weights('./pretrain_model/nezha2/q2q1nezha5e-5testB.weights')
        print(
...
if __name__ == '__main__':
    ...
    evaluator = Evaluator()
#     model.load_weights('./pretrain_model/nezha2/nezha_best_model.weights')
    model.fit(
